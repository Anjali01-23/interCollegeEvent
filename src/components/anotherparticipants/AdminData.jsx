...
const AdminData = () => {
  ...
  const filteredRequests = (requests || []).filter((req) => {
    ...
    const text = `${req.college || ""} ${req.email || ""}`.toLowerCase();
    ...
    <div className="p-4 text-gray-100">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-sm text-gray-400">View and filter all admin requests</p>
        </div>

        <div className="flex items-center gap-2">
          <input
            type="text"
            placeholder="Search College Name / email "
            value={search}
            ...
            <table className="min-w-full table-auto">
              <thead className="bg-gray-900 text-left text-sm text-gray-200">
                <tr>
                  ...
                  <th className="p-4 border-b border-gray-700">College Name</th>
              ...
              <tbody>
                {filteredRequests.map((req) => (
                  <tr key={req.id} className="hover:bg-gray-900">
                    ...
                    <td className="p-3 text-sm border-b border-gray-700 text-gray-100">{req.email}</td>
                    <td className="p-3 text-sm border-b border-gray-700 text-gray-300">{req.college}</td>
                    <td
                      className={`p-3 text-sm border-b border-gray-700 font-semibold ${
                        req.status === "approved" ? "text-green-400" : req.status === "rejected" ? "text-red-400" : "text-yellow-400"
                      }`}
                    >
