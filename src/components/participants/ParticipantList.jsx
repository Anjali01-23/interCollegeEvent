  ...
  return (
    <div className="pt-0 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-lg sm:text-xl font-semibold mb-3">Participants List</h2>
        ...
        {/* States */}
        {loading ? (
          <div className="rounded-lg bg-white p-6 text-center text-gray-600">Loading participants…</div>
        ) : error ? (
          <div className="rounded-lg bg-red-50 border border-red-200 p-4 text-red-700">{error}</div>
        ) : filteredParticipants.length === 0 ? (
          <div className="rounded-lg bg-white p-6 text-center text-gray-600">No participants found</div>
        ) : (
          <>
            {/* Desktop table */}
              ...
              <table className="min-w-full table-auto">
                ...
                <tbody>
                  {filteredParticipants.map((p) => (
                    <tr key={p.id ?? `${p.event_id}_${p.email}`} className="hover:bg-gray-50">
                      <td className="p-3 text-sm border-b">{p.name ?? "-"}</td>
                      <td className="p-3 text-sm border-b">{p.email ?? "-"}</td>
                      <td className="p-3 text-sm border-b">{p.event_name ?? p.event_title ?? "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="md:hidden space-y-3">
              {filteredParticipants.map((p) => (
                <div key={p.id ?? `${p.event_id}_${p.email}`} className="bg-white p-4 rounded-lg shadow-sm border">
                  <div className="flex justify-between items-start">
                    <div className="min-w-0">
                      <p className="font-medium text-gray-800 truncate">{p.name ?? "Unknown"}</p>
                      <p className="text-sm text-gray-500 truncate">{p.email ?? "-"}</p>
                      <p className="text-sm text-gray-500 mt-1">
                        Event: <span className="font-medium text-gray-700">{p.event_name ?? p.event_title ?? "N/A"}</span>
                      </p>
                    </div>
                  </div>
                </div>
